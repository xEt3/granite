...
export default class JobOpening extends Model {
  ...
  @attr('number')
  internalDuration; // number of days that this job is internally available before posting to outside sources

  @attr('number', { defaultValue: 1 })
  positions; // number of positions to fill

  @attr('boolean')
  sendCloseNotice; // send an email to unrejected talent when job closes

  @attr('boolean')
  allocateTalentPool; // allocate unrejected, not hired talent to the pool
  ...
  @hasMany('applicant-source')
  applicantSources;

  @hasMany('manual-applicant-source')
  manualApplicantSources;
